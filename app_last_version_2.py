...
if st.session_state.script1_done:
    ...
    for bf_file, pl_file in zip(bf_files, pl_files):
        ...
        # Calculate total cells from the cells dataframe
        total_cells = region_area_df["Region_Label"].count()

        # Calculate total distinct cells that have crystals (from df_mapping)
        #total_distinct_cells = df_mapping["Total_Cells_with_crystals"]
        total_distinct_cells = int(df_mapping["Total_Cells_with_crystals"].iloc[0])
        ...
        summary_df = pd.DataFrame([{
            ...
            "Percentage_Cells_with_Crystals": percentage
        }])
    ...
    summary_df.to_excel(excel_path_2, index=False)
...
if st.session_state.script1_results:
    ...
    with open(excel_path_2, "rb") as f2:
        st.download_button("📊 Download Dataset Plot",f2,file_name=os.path.basename(excel_path_2),key="download_plot_data")
#-----------------------------------------------------------------------------------------------------------------------------------
